...
window.addEventListener('beforeunload', function () {
  ...
  const jsonBox = document.querySelector('.json-reader'); // give your div a class
  if (jsonBox) {
    sessionStorage.setItem('jsonScroll', jsonBox.scrollTop);
  }
});

window.addEventListener('load', function () {
  const pageScroll = sessionStorage.getItem('pageScroll');
  if (pageScroll !== null) {
    window.scrollTo(0, parseInt(pageScroll));
  }

  const jsonBox = document.querySelector('.json-reader');
  const jsonScroll = sessionStorage.getItem('jsonScroll');
  if (jsonBox && jsonScroll !== null) {
    jsonBox.scrollTop = parseInt(jsonScroll);
  }
});
</script>