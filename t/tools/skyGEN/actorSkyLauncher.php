...
require_once __DIR__ . '/-CRATE-skyGEN.php'; // CRATE FILLER SETTINGS
require_once __DIR__ . '/functions.php';


if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    //definers::
      define("POST_SURFACE", $_POST['POST_SURFACE']);
      define("POST_BUILDING", $_POST['POST_BUILDING']);
      define("POST_MOD", $_POST['POST_MOD']);
      define("POST_TIMEZONE", $_POST['POST__TZ']);
...
//  BET ROUTE__LINE >>>>>>>>>>>>>>>>>>>>>>>>
$ROUTE = B_ROUTE . $_POST['SYS_SLUG'] . '/';

  if (is_dir($ROUTE)) { KDE_Error_Logger('skyGEN', 'World Exists!');}
    else { mkdir($ROUTE, 0775, true); }
        
    $genSKY_AUTH = CREATE_SKY_AUTH();
    $storeSKY_AUTH = $ROUTE . '-SKY_AUTH-' . str_replace("/", "-", $_POST['gen-WORLD_SLUG']) . '.php';
    file_put_contents($storeSKY_AUTH, $genSKY_AUTH);

    $CREATED_SKY_SIG = CREATE_SKY_SIG();
    $SKY_SIG = $ROUTE . '-SKY_SIG-' . str_replace("/", "-", $_POST['gen-WORLD_SLUG']) . '.php';
...
// IMPROVEMENTS END HERE -- KEEP WORKING FROM THIS POSITION

//  KHAF ROUTE LINE >>>>>>>>>>>>>>>>>>>>>>>>
$ROUTE__LINE = ROUTE("c", SHADOW_TOGGLE);
...
//  MEM ROUTE LINE >>>>>>>>>>>>>>>>>>>>>>>>
$ROUTE__LINE = ROUTE("m", SHADOW_TOGGLE);
...
//  ALEPH ROUTE LINE >>>>>>>>>>>>>>>>>>>>>>>>
$ROUTE__LINE = ROUTE('a', SHADOW_TOGGLE);
